...
- **Radio control** - Turn SDR radios on/off and report AIOv2/RTL-SDR state
- **Arduino button/touchscreen support** - Event queue for hardware controls
- **SDRDecoder plugin** - Launches the external satellite ground-station web app on port 8888
...
| `/api/diagnostics/ac1200` | Read-only AC1200/MT7921 hardware and driver diagnostics; optional `?wifi_iface=wlan1` |
...
| `/api/radio/frequency` | Reserved for a tuner-specific backend; currently returns unsupported |

AC1200 diagnostics are also available locally without the API:

```bash
k7bat-ac1200-diagnostics --json ~/.config/k7bat-uconsole-status/diagnostics/ac1200.json \
  --markdown ~/.config/k7bat-uconsole-status/diagnostics/ac1200.md
```

The command performs read-only USB, driver, RFKILL, firmware-evidence, and Wi-Fi capability checks. It does not enable monitor mode, change interfaces, or start scans.

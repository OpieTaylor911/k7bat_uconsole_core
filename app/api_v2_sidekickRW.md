...
### Status App version discovery

Sidekick firmware can check the installed uConsole Status App with:
...
Use `api_version` for the HTTP contract and `status_app_version` for the installed dashboard/application version. Do not compare the two fields as if they were the same release number.
